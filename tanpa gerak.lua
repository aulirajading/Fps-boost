--[[
    CVAI - SAFE FREECAM (No Freeze, No Detection)
    Fitur:
    - Freecam (J) - WASD gerak kamera, karakter DIAM (input diblokir)
    - ESP HANYA MUSUH (L) - Update 0.5 detik
    - Zoom 400 (Z)
    - Teleport (ALT + Klik Kiri)
]]
...
local state = {
    ...
    originalAutoRotate = nil  -- Untuk simpan nilai asli
}
...
-- ========== TELEPORT ==========
local function getGroundPosition(mouseX, mouseY)
    local result = nil
    pcall(function()
...
local function teleportQuick()
    ...
    local groundPos = getGroundPosition(mousePos.X, mousePos.Y)
...
local function startFreecam()
    ...
    -- ==== BLOKIR INPUT KARAKTER (TANPA FREEZE) ====
    if state.humanoid then
        -- Matikan auto rotate (biar karakter tidak ikut arah kamera)
        state.humanoid.AutoRotate = false
    end
    -- TIDAK menyentuh PlatformStand! Biarkan false
    ...
    print("[CVAI] Freecam ON - Karakter DIAM (input diblokir), tapi bisa jatuh/terguling")
end

local function stopFreecam()
    if not state.freecamActive then return end
    
    -- ==== KEMBALIKAN INPUT KARAKTER ====
    if state.humanoid and state.originalAutoRotate ~= nil then
    ...
    print("[CVAI] Freecam OFF - Karakter normal")
end
...
print("[CVAI] ========== SAFE FREECAM ==========")
print("[CVAI] J = Freecam (Karakter DIAM - input diblokir)")
print("[CVAI] L = ESP (HANYA MUSUH)")
print("[CVAI] Z = Zoom 400")
print("[CVAI] ALT + Klik Kiri = Teleport")
print("[CVAI] ==================================")
print("[CVAI] Karakter TIDAK di-freeze!")
print("[CVAI] - Bisa jatuh/terguling (physics tetap jalan)")
print("[CVAI] - TAPI tidak gerak karena WASD untuk kamera")
